...
public class Buffers {
    ...
    public FloatBuffer toFloatBuffer(V8Array array){
        ...
        FloatBuffer b = possibleBufferHolder.getBuffer();
        for (int i = 0; i < size; i++) {
            b.put(i,(float)array.getDouble(i));
        }
        possibleBufferHolder.rewind();
        return possibleBufferHolder.getBuffer();
    }

    public IntBuffer toIntBuffer(V8Array array){
        int size = array.length();
    ...
    public ShortBuffer toShortBuffer(V8Array array){
        int size = array.length();
    ...
    public ByteBuffer toByteBuffer(V8Array array){
        int size = array.length();
        ...
        for (int i = 0; i < size; i++) {
            possibleBuffer.put(i,(byte) array.getInteger(i));
        }
        possibleBuffer.position(0);
